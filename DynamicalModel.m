function [state_derivative, state_out,friction_forces,force] = DynamicalModel(voltages, state, stepSize_time)
...
u = state(13);    % forward velocity,    v, m/s
v = state(14);   
w = state(15);     
p = state(16);
q = state(17);
r = state(18);      %rotational velocity, r, rad/s
xx = state(19);     % xx because the input matrix is x
y = state(20);      %current y-position,  y, m
z = state(21);
phi = state(22);
theta = state(23);
psi = state(24);     %heading angle, psi, rad
...
force = [force_l1,force_l2,force_r1,force_r2]; 
%% Slip
...
% Assign friction forces to output
friction_forces = [Fx_fric, Fy_fric, Fz_fric, K_fric, M_fric, N_fric]; 
% Air Resistance
% x-axis
